...
setInterval(function(){
    ...
    request.get(config.rtmp_server_url, {
        timeout:config.rtmp_server_timeout
    }, function(error, meta, xml){

    if(error) {
        // console.log('********** request stat fail.');
        io.emit('error', {"error":true});
    } else {
        // console.log('********** request stat success.');
    }
    ...
    parseString(xml,function (err, result) {
    	...
    	if(err != null)
    	{
    		io.emit('error', {"error":true, "result": result});
    	}else{
    		io.emit('error', {"error":false, "result": result});
    	}
        ...
        if(err != null)
        {
            ...
            // console.log('emit statistics, ' + result.rtmp + ', result-http-flv:' + result['http-flv'])
            io.emit('statistics', result['http-flv']);
        }
